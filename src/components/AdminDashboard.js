...
const AdminDashboard = () => {
  ...
  // 🔹 BookingContext for updating status
  ...
  // 🔹 Update Booking Status via BookingContext
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      // Use the updated BookingContext function which handles missing documents
        ...
        <table>
          ...
          <tbody>
            {users.length > 0 ? (
              users.map((user) => (
                <tr key={user.id}>
                  <td>{user.id}</td><td>{user.email || "No Email"}</td>
                  <td>
          ...
          <tbody>
            {filteredBookings.length > 0 ? (
              filteredBookings.map((booking) => (
                <tr key={booking.id}>
                  <td>{booking.id}</td><td>{booking.userEmail || "Unknown"}</td><td>{booking.name || "N/A"}</td>
                  <td>
